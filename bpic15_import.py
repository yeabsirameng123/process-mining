...
driver = GraphDatabase.driver("bolt://localhost:7687", auth=("neo4j", "53787jjj"))
...
path_to_neo4j_import_directory = 'C:\\Users\\yeabs\\Downloads\\neo4j-community-3.5.35-windows\\neo4j-community-3.5.35\\import\\'
...
def CreateEventQuery(logHeader, fileName, LogID = ""):
    ...
    for col in logHeader:
        ...
        if col == 'org_resource':
            column = 'line["org_resource"]'
        elif col == 'Responsible_actor':
            column = 'line["Responsible_actor"]'
        elif col == 'Includes_subCases':
            column = 'line["Includes_subCases"]'
        elif col == 'last_phase':
            column = 'line["last_phase"]'
        elif col == 'case_type':
            column = 'line["case_type"]'
        if col == 'idx':
...
def createEventClass_Activity(tx):
    qCreateEC = f'''
        MATCH (e: Event) WITH DISTINCT e.Activity AS actName WHERE actName IS NOT NULL
...
def createEventClass_ActivityANDLifeCycle(tx):
    qCreateEC = f'''
        ...
        WHERE actName IS NOT NULL AND lifecycle IS NOT NULL
...
def createEventClass_Resource(tx):
    qCreateEC = f'''
        MATCH (e: Event) WITH DISTINCT e.resource AS name WHERE name IS NOT NULL
...
def delete_nodes_and_relationships_in_batches(driver, batch_size):
    deleted_nodes = 0
    deleted_relationships = 0
    while True:
        with driver.session() as session:
            # Delete relationships in batches
            result = session.run("MATCH ()-[r]-() WITH r LIMIT $batchSize DELETE r RETURN count(r) as deletedCount", 
                                 {'batchSize': batch_size})
            deleted_rel_count = result.single()[0]
            deleted_relationships += deleted_rel_count

            if deleted_rel_count == 0:
                # If no more relationships, start deleting nodes
                result = session.run("MATCH (n) WITH n LIMIT $batchSize DELETE n RETURN count(n) as deletedCount", 
                                     {'batchSize': batch_size})
                deleted_node_count = result.single()[0]
                deleted_nodes += deleted_node_count

                if deleted_node_count == 0:
                    # Exit loop if no more nodes to delete
                    break

    print(f"Deleted {deleted_relationships} relationships and {deleted_nodes} nodes in batches.")

# Usage
if step_ClearDB:
    print('Clearing DB...')
    delete_nodes_and_relationships_in_batches(driver, 1000)  # Adjust batch_size as needed
...
for fileName in logfiles:
    ...
    if step_LoadEventsFromCSV:
        ...
        new_row = pd.DataFrame([{'name': dataSet+'_event_import', 'start': last, 'end': end, 'duration': (end - last)}])
    ...
    if step_createLog:
        ...
        new_row = pd.DataFrame([{'name': dataSet+'_create_log', 'start': last, 'end': end, 'duration': (end - last)}])
        perf = pd.concat([perf, new_row], ignore_index=True)
        print('Log and :L_E relationships done: took '+str(end - last)+' seconds')
        last = end  
    ...
    if step_createEntities:
        for entity in model_entities: # per entity
            ...
            with driver.session() as session:
            # Call create_entity with the correct number of arguments
                session.execute_write(create_entity, entity[0], entity[1], entity[2])
                print(f'{entity[0]} entity nodes done')
            
            # Call correlate_events_to_entity with the correct number of arguments
                session.execute_write(correlate_events_to_entity, entity[0], entity[1], entity[2])
                print(f'{entity[0]} E_EN relationships done')

        
            end = time.time()
            new_row = pd.DataFrame([{'name': dataSet+'_create_entity ' + entity[0], 'start': last, 'end': end, 'duration': (end - last)}])
            perf = pd.concat([perf, new_row], ignore_index=True)
    ...
    if step_createDF:
        ...
        for entity in all_entities: #per entity
            with driver.session() as session:
                session.execute_write(createDirectlyFollows, entity, option_DF_entity_type_in_label)

                
            end = time.time()
            new_row = pd.DataFrame([{'name': dataSet+'_create_df ' + entity, 'start': last, 'end': end, 'duration': (end - last)}])
            perf = pd.concat([perf, new_row], ignore_index=True)
    ...
    if step_createEventClasses:
            ...
            new_row = pd.DataFrame([{'name': dataSet+'_create_classes', 'start': last, 'end': end, 'duration': (end - last)}])
            perf = pd.concat([perf, new_row], ignore_index=True)
    ...
    if step_createDFC:
        ...
        for entity in all_entities:
            ...
            new_row = pd.DataFrame([{'name': dataSet+'_aggregate_df_' + entity, 'start': last, 'end': end, 'duration': (end - last)}])
            perf = pd.concat([perf, new_row], ignore_index=True)
    ...
    # First Section
        ...
        new_row = pd.DataFrame([{'name': dataSet+'_create_how', 'start': start, 'end': end, 'duration': (end - last)}])
        perf = pd.concat([perf, new_row], ignore_index=True)
        print('Creating HOW network done: took ' + str(end - last) + ' seconds')
        last = end
...
# Second Section
    ...
    new_row = pd.DataFrame([{'name': dataSet+'_total', 'start': start, 'end': end, 'duration': (end - start)}])
    perf = pd.concat([perf, new_row], ignore_index=True)
