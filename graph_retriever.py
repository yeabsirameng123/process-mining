...
class GraphRetriever:
    ...
    def fetch_eckg_graph(self):
        ...
        #query="MATCH p=()-[r:DF_C]->() RETURN p LIMIT 25"
        with self.driver.session() as session:
            result = session.run(query)
    ...
    def get_events_of_class(self,id_class):
        query = f"MATCH (c:Class {{ID: '{id_class}'}})<-[:OBSERVED]-(e:Event) RETURN COUNT(e) AS event_count"
        with self.driver.session() as session:
            result = session.run(query)
            return result.single()["event_count"]
